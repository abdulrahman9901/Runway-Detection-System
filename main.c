...
#define SERVER_IP_ADDRESS "192.168.0.100" // Change this to your server's IP address
...
//	const char* pcHost = "127.0.0.1";  // Replace with the IP address of your Express server
//	const char* pcMessage = "Hello, this is a text message!";
//	const char* pcPageFormat = "/?message=%s";  // Include the message in the query parameters
//	const char* pcPage = "/";
//	const int port = 3000;  // Default HTTP port
//
//	struct freertos_sockaddr xRemoteAddress;
//	BaseType_t xBytesReceived;
//	char pcBuffer[512]; // Adjust the buffer size as needed
//
//	/* Set the IP address of the remote server. */
//	xRemoteAddress.sin_port = FreeRTOS_htons(port);
//	xRemoteAddress.sin_addr = FreeRTOS_inet_addr_quick(127, 0, 0, 1);  // Replace with the IP address of your Express server
//
//	/* Create a socket. */
...
//		const char* pcHTTPRequestFormat = "GET %s HTTP/1.1\r\nHost: %s\r\n\r\n";
//		
//		size_t xRequestLength = strlen(pcHTTPRequestFormat) + strlen(pcPage) + strlen(pcHost) + strlen(pcPageFormat) + strlen(pcMessage) - 4;
//		char* pcHTTPRequest = (char*)pvPortMalloc(xRequestLength);
//
//		if (pcHTTPRequest != NULL) {
//			snprintf(pcHTTPRequest, xRequestLength, pcPageFormat, pcHTTPRequestFormat, pcPage, pcHost );
//
//			/* Send the HTTP request. */
//			BaseType_t xBytesSent = FreeRTOS_send(xSocket, pcHTTPRequest, xRequestLength, 0);
...
//				while ((xBytesReceived = FreeRTOS_recv(xSocket, pcBuffer, sizeof(pcBuffer) - 1, 0)) > 0) {
//					pcBuffer[xBytesReceived] = '\0';
//					FreeRTOS_printf(("Received: %s", pcBuffer));
//				}
//			}
//			else {
//				FreeRTOS_printf(("Send error: %ld\n", xBytesSent));
//			}
//
//			vPortFree(pcHTTPRequest);
//		}
//		else {
//			FreeRTOS_printf(("Memory allocation error\n"));
//		}
//	}
//	else {
//		FreeRTOS_printf(("Connection error\n"));
//	}
//
//	/* Close the socket. */
...
static void socketTask()
{

	/* Delay to give time to the FreeRTOS network interface to open*/
	vTaskDelay(1500);
	printf("Socket Task - starting\n");

	/* Variable to hold the created socket. */
	Socket_t xSocket;
	struct freertos_sockaddr xRemoteAddress; /*< Ip address of the server*/
	...
	{
		printf("socketTask - Could not open socket. Closing task\n");
		return;
	}

	/* Fill in the destination address and port number, which in this case is
	port 5030 on IP address 192.168.0.100 */
	xRemoteAddress.sin_addr = FreeRTOS_inet_addr_quick(IP_1, IP_2, IP_3, IP_4);
	xRemoteAddress.sin_port = FreeRTOS_htons(PORT);

	/* Connect to remote address*/
	int conectedResult = FreeRTOS_connect(xSocket, &xRemoteAddress, sizeof(xRemoteAddress));
	vTaskDelay(25);
	char pvBuffer[10]="hello\n";
	const char* pcPage = "/";
	const char* pcHost = "127.0.0.1";
	const char* pcParam1 = "name=John";
	const char* pcParam2 = "age=25";

	// Construct the URL with parameters
	const char* pcURLFormat = "GET %s?%s&%s HTTP/1.1\r\nHost: %s \r\n\r\n";
	size_t xURLLength = strlen(pcURLFormat) + strlen(pcPage) + strlen(pcParam1) + strlen(pcParam2) + strlen(pcHost);
	char* pcURL = (char*)pvPortMalloc(xURLLength);

	if (pcURL != NULL) {
		snprintf(pcURL, xURLLength, pcURLFormat, pcPage, pcParam1, pcParam2, pcHost);

		// Send the HTTP GET request with parameters
		BaseType_t xBytesSent = FreeRTOS_send(xSocket, pcURL, strlen(pcURL), 0);

		printf("Socket task - Sending %d bytes ... \n", xBytesSent);
	}
...
void vApplicationIPNetworkEventHook(eIPCallbackEvent_t eNetworkEvent) {
	if (eNetworkEvent == eNetworkUp) {
		/* Create a task to perform the HTTP request when the network is up. */
		xTaskCreate(socketTask, "HTTPRequest", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
	}
}
int main( void )
{
...
TimerHandle_t xCheckTimer;
	...
	***NOTE*** Tasks that use the network are created in the network event hook
	...
	// Assuming your data is in a string format, for example, a JSON payload
	const char* pcDataToSend = "{\"key\": \"value\", \"temperature\": 25.5}";

	// Call the function to send data to the server
	// vTCPSendDataToServer("localhost", "/data", pcDataToSend);
